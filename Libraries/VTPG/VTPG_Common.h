...
#define LOG_EXPR(_X_) do{\
	...
	if(_STR_)\
		...
		_NSLog(@"Unknown _TYPE_CODE_: %s for expression %s in function %s, file %s, line %d", _TYPE_CODE_, #_X_, __func__, __FILE__, __LINE__);\
}while(0)

#define LOG_NS(...) _NSLog(__VA_ARGS__)
#define LOG_FUNCTION()	_NSLog(@"%s", __func__)
